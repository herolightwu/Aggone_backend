...
class AlarmController extends APIController
{
    ...
    public function get_all_notifications(Request $request){
        ...
        $notifications = Alarm::query()->where('user_id', $user->id)->with('sender')->get();
    ...
    public function push_chat_notification(Request $request){
        ...
        if(isset($other)){
            ...
            $dd = Alarm::query()->whereRaw($cond)->first();
            if (isset($dd)){
                $dd = Alarm::find($dd->id);
                $dd->delete();
            }
            $dd = Alarm::create($noti);
            $pusher = new PushController();
            $ret = $pusher->sendPush([$other->id], $content, $dd);
            return response()->json(['status' => true], $this->successStatus);
        }

        return response()->json(['status' => false], $this->failedStatus);
    }
}
